...
def image_callback(ros_image):
  ...
  contourDraw = [c for c in contours if cv2.contourArea(c)>100]
...
#    centroid2 = get_centroid(max_c2)
    ...
    for i in range(len(sorted_contours)):
      if(i<5 and cv2.contourArea(sorted_contours[i]) > 10):
  ...
  contours_extended = [c for c in contours_extended if cv2.contourArea(c)>100]

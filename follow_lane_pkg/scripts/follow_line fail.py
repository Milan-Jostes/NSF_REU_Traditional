...
def find_centroids(temp):
  ...
  if last1 is not None and last2 is not None:
    ...
    right_x = last2[0]
    for x in centroids:
      left_dist.append(abs(x[0] - left_x-(x[1] - last1[1])*0.5))
      right_dist.append(abs(x[0] - right_x-(x[1] - last2[1])*0.5))
    max1=centroids[np.argmin(left_dist)]
    max2=centroids[np.argmin(right_dist)]
    limit = 0
    while limit <5:
      if abs(max1[0]-max2[0]) <250:
...
def draw_and_extend_contour_straight(cv_image, contour, color=(255,255,255), thickness=20):
    ...
    if(abs(end[0]-start[0])!=0):
      if (math.atan(abs(end[1]-start[1])/abs(end[0]-start[0]))*180/math.pi)>45:
...
def preprocess(img):
    ...
    con_test = cv2.convertScaleAbs(con,alpha=1.0,beta=20)
    ...
    cv_image = cv2.drawContours(makeimg, contour_sized, -1, (0, 0, 255), thickness=-1)
    return cv_image
